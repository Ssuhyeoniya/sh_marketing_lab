...
export async function extractPageTextItems(pdf, pageNum, canvasScale = 2) {
  ...
  // pdfjs already groups text by content-stream segments — typically one
  // item per word or sub-line. Trying to re-split here on whitespace via
  // measureText is unreliable: when the PDF's embedded font isn't loaded
  // yet, measureText falls back to system sans, and the proportional widths
  // shift words by 5-30 px. Leaving items as pdfjs gave them preserves the
  // exact original bbox.
